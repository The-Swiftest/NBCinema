...
class SignUpView: UIView {
    ...
    let passwordInput = NbcInputField(placeholder: "비밀번호", type: .newPassword)
    let confirmPasswordInput = NbcInputField(placeholder: "비밀번호 확인", type: .newPassword)
    
    // 회원가입 버튼
    let signUpButton = NbcButton(title: "회원가입")
    ...
    // 로그인 페이지 이동 버튼
    let moveToLoginButton = UIButton().then {
    ...
    private func setupUI() {
        ...
        [questionLabel, moveToLoginButton].forEach {
            questionStackView.addArrangedSubview($0)
        }
        [logoImageView, emailInput, passwordInput, confirmPasswordInput, signUpButton, questionStackView].forEach {
            addSubview($0)
        }
        ...
        confirmPasswordInput.snp.makeConstraints {
            $0.top.equalTo(passwordInput.snp.bottom).offset(15)
            $0.leading.trailing.equalTo(safeAreaLayoutGuide).inset(40)
        }
